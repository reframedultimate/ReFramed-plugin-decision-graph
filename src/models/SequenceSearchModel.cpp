...
void SequenceSearchModel::startNewSession(const rfcommon::MappingInfo* map, const rfcommon::Metadata* mdata)
{
    ...
    for (int s : unmapped)
    {
        ...
        auto fighterData = fighterStates_.push({
            mdata->playerTag(s),
...
bool SequenceSearchModel::applyQueryNoNotify(int queryIdx)
{
    ...
    const auto& states = fighterStates_[currentFighterIdx_];
    ...
    {
        const auto& sessionSeq = sessions_[sessionIdx].fighterStatesRange[currentFighterIdx_];
